     ...
     <tr>
         ...
         <input type="hidden" value="${entity.fdId}"/>
         	<c:if test="${empty entity.fdAdvice}">
	         	...
	         	<a class="btn_feedback edit_my_application" id="apply_${entity.fdId}" onclick="showUpdate($(this).attr('id'))">修改</a>
	         	</c:if>
         		<c:if test="${entity.fdStatus ne 0}">${entity.fdAdvice}</c:if>
         	</c:if>
