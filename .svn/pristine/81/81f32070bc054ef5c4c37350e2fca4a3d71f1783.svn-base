...
public class TaskController extends BaseController
{
	...
	/** admin所选的该用户的正常任务**/
	@RequestMapping("/listnormal_admin")
	public String listAdminNormalByUser(Integer userId, Integer taskcurrentpage, Integer taskselectPageSize, Model model, HttpServletRequest request)
	{
		if(null == taskcurrentpage || taskcurrentpage < 1)
		{
			taskcurrentpage = 1;
	...
	/** admin所选的该用户已被分配的所有任务**/
	@RequestMapping("/listgive_admin")
	public String listAdminGiveByUser(Integer userId, Integer taskGivecurrentpage, Integer selecttaskGivePageSize, Model model, HttpServletRequest request)
	{
		...
		model = taskService.queryOverNormalList(tasks.getResultlist(), model);
	...
	public String listGiveByUser(Integer userId, Integer taskGivecurrentpage, Integer selecttaskGivePageSize, Model model, HttpServletRequest request)
	{
		...
		roles = userRoleService.findRoleByUserId(userId);
		for(Role r : roles)
		{
			if("dest".equals(r.getFdRoleKey()))
			{
				PageView<Task> taskGivePageView = taskService.queryGiveList(taskGivecurrentpage, selecttaskGivePageSize);
				model = taskService.queryOverNormalList(taskGivePageView.getRecords(), model);
				model.addAttribute("taskGivePageView", taskGivePageView);
	...
	/** 查询全部分配任务**/
	@RequestMapping(value = "/giveall_dest")
	public String listGiveAllToDest(Integer taskGivecurrentpage, Integer selecttaskGivePageSize, Model model)
	{
		if(null == taskGivecurrentpage || taskGivecurrentpage < 1)
		{
			taskGivecurrentpage = 1;
